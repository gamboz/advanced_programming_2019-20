...
class List {
 ...
 public:
  ...
  using iterator       =  pippo<T>;
  using const_iterator =  pippo<const T>;
...
public:
  explicit pippo( node* x ) noexcept : current{x} {}


  // aliases so that the stl algos can work with this iterator
  using value_type = O;
  using difference_type = std::ptrdiff_f;
  ...
  // pre-increment
  pippo& operator++() noexcept {
    current = current->next.get();
    return *this;
  }

  // post-increment uses copy by value
  // not needed to implement the behaviour of an iterator

  friend
  bool operator==(const pippo& a, const pippo& b) { return a.;  }
  bool operator!=(const pippo& a, const pippo& b) { return ! a == b; }
